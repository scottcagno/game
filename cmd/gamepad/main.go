...
func printStuff(g *Game) string {
	ss := fmt.Sprintf("gamepadIDsBuf: %d\n", len(g.gamepadIDsBuf))
	ss += fmt.Sprintf("gamepadIDs: %d\n", len(g.gamepadIDs))
	ss += fmt.Sprintf("axes: %d\n", len(g.axes))
	ss += fmt.Sprintf("pressedButtons: %d\n", len(g.pressedButtons))
	return ss
}
...
`
	...
	m += fmt.Sprintf("    Left Stick:  X: %#v, Y: %#v\n    Right Stick: X: %+0.2f, Y: %+0.2f",
		int(ebiten.StandardGamepadAxisValue(id, ebiten.StandardGamepadAxisLeftStickHorizontal)),
		int(ebiten.StandardGamepadAxisValue(id, ebiten.StandardGamepadAxisLeftStickVertical)),
